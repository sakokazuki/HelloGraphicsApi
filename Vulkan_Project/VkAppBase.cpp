...
VulkanAppBase::VulkanAppBase(): m_presentMode(VK_PRESENT_MODE_FIFO_KHR), m_imageIndex(0)
{
}
...
void VulkanAppBase::initialize(GLFWwindow* window, const char* appName)
{
	...
	// GLFWにある便利関数を使用してサーフェスVkSurfaceKHRを作成
	// vkInstanceを使用する
	glfwCreateWindowSurface(m_instance, window, nullptr, &m_surface);
	// サーフェスのフォーマットを指定
	selectSurfaceFormat(VK_FORMAT_B8G8R8A8_UNORM);

	// VkSurfaceCapabilitiesKHR(サーフェスの能力値)を取得
	// VkSurfaceKHRとVkPhysicalDeviceが必要
	// 能力値の中にサイズなどの情報が入っている
...
///
/// Vulkanのインスタンスを作成、作成にはVkInstanceCreateInfoが必要で
/// VkInstanceCreateInfoの作成にVkApplicationInfoが必要
///
void VulkanAppBase::initializeInstance(const char* appName)
{
	vector<const char*> extensions;
	// app名/ エンジン名/ API version/ エンジンバージョン
	...
	// エクステンション、デバッグレイヤー vkApplicationInfoを設定
...
///
/// 物理デバイスの作成
/// インスタンスが必要、vkGetPhysicalDviceMemoryPropertiesは物理デバイスから
/// メモリの情報を取得する
///
...
///
/// グラフィックスキューの場所を検索。
/// 物理デバイスからキュープロパティの数を取得してからVkQueueFamilyProperiesの配列に
/// データを格納。それの中からグラフィックスキューを見つけたらその場所を記録する
/// グラフィックスキュー以外のキューはいずれ使う？
///
...
///
/// 論理デバイスvkDeviceの作成。まずグラフィックスキューを利用して
/// vkDeviceQueueCreateInfoを作成する。
/// 次に物理デバイス(VkPhsycalDevice)から使用可能なエクステンションを取得して
/// Queueinfo, エクステンションを使ってVkDeviceCreateInfoを生成、vkDevideを作成する。
/// ついでにvkDeviceとvkPhisycalDeviceとグラフィックスキューからデバイスキューも取得する
///
...
///
/// コマンドプール作成。コマンドプールはコマンドバッファを保持するためのもの
/// vkCommandPoolCreateInfoを使用して作成する。作成にはvkDevice, グラフィックスキューのインデックス
/// が必要。flagsは上級者設定っぽい毎回リセットしないほうが早くなるらしいが本ではリセットを使用
///
...
///
/// 物理デバイス、サーフェスを用いて使用できるフォーマットを取得
/// その中から一致するものを取得する
///
...
///
/// スワップチェインの作成
/// 基本的にはVkSurface~~KHRシリーズの情報から取得できる
/// 情報多いがほとんど解説もなかったのでこれが定石なのかもしれない
///
