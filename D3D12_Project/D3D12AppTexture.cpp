...
void D3D12AppTexture::Prepare()
{
	...
	hr = CompileShaderFromFile(L"textureVS.hlsl", L"vs_6_0", m_vs, errBlob);
	if (FAILED(hr))
	{
		OutputDebugStringA((const char*)errBlob->GetBufferPointer());
	}
	hr = CompileShaderFromFile(L"texturePS.hlsl", L"ps_6_0", m_ps, errBlob);
	if (FAILED(hr))
	{
		OutputDebugStringA((const char*)errBlob->GetBufferPointer());
	}
...
void D3D12AppTexture::MakeCommand(ComPtr<ID3D12GraphicsCommandList>& command)
{
...
D3D12AppTexture::ComPtr<ID3D12Resource1> D3D12AppTexture::CreateTexture(const std::string& fileName)
{
...
void D3D12AppTexture::PrepareDescriptorHeapForCubeApp()
{
	// CBV/SRV のディスクリプタヒープ
	//  0:シェーダーリソースビュー
